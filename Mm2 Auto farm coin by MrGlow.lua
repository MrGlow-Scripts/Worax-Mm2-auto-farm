--[=[ MM2 ULTIMATE SCRIPT by MrGlow - ВСЕ ФУНКЦИИ ВКЛЮЧЕНЫ, КНОПКА ИСПРАВЛЕНА ]=]
...
-- === НАСТРОЙКИ (ВСЕ ФУНКЦИИ ВКЛЮЧЕНЫ) ===
local CONFIG = {
    FarmSpeed = 0.1,
    AntiAFK = true,
    AutoTeleport = true,
...
local function collectLoop()
    while isFarming do
        ...
        for _, item in ipairs(items) do
            ...
            if not checkAlive() then
                isDead = true                statusLabel.Text = lang.StatusDead
...
print("[MM2 Ultimate] by MrGlow загружен! Кнопка работает.")
